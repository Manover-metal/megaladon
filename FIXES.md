...
**Проблема:**
Не было правила `unique:users,phone` в валидации. Проверка на дубликат была только на уровне сервиса (`getUserByPhone`), без защиты на уровне базы данных. При параллельных запросах оба могли пройти проверку.
...
## SMS-сервис (требует подключения)
